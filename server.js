...
const dbConnectionPromise = mongoose.connect(MONGO_URI).then(m => { console.log('成功连接到 MongoDB Atlas'); return m.connection.getClient(); }).catch(err => console.error('连接 MongoDB 失败:', err));

const sessionSchema = new mongoose.Schema({ name: { type: String, required: true }, code: { type: String, required: true, unique: true, default: () => nanoid(6) }, totalConnections: { type: Number, default: 0 }, createdAt: { type: Date, default: Date.now } });
const Session = mongoose.model('Session', sessionSchema);
const questionSchema = new mongoose.Schema({ text: String, name: { type: String, default: '匿名' }, sessionId: { type: mongoose.Schema.Types.ObjectId, ref: 'Session', required: true }, createdAt: { type: Date, default: Date.now } });
...
app.use(session({
    ...
    store: MongoStore.create({ clientPromise: dbConnectionPromise, collectionName: 'user_sessions', ttl: 14 * 24 * 60 * 60, autoRemove: 'native' }),
    cookie: { secure: process.env.NODE_ENV === 'production', maxAge: 1000 * 60 * 60 * 24 * 7 }
}));

const checkAuth = (req, res, next) => {
    if (req.session.isAuthenticated) { return next(); }
    res.status(403).send('Forbidden: Please log in using your secret link or clear your cookies if you are stuck.');
};

app.get('/login/:secretPath', (req, res) => {
...
app.get('/logout', (req, res) => {
    req.session.destroy(err => {
        ...
        res.send('You have been logged out. Please use your secret link to log in again.');
    });
});
...
const rooms = new Map();
wss.on('connection', (ws) => { ws.on('message', async (message) => { try { const data = JSON.parse(message); if (data.type === 'join' && data.room) { const roomCode = data.room; ws.roomCode = roomCode; if (!rooms.has(roomCode)) { rooms.set(roomCode, new Set()); } rooms.get(roomCode).add(ws); console.log(\`一个客户端加入了房间: \${roomCode}\`); await Session.findOneAndUpdate({ code: roomCode }, { $inc: { totalConnections: 1 } }); const currentCount = rooms.get(roomCode).size; broadcastToRoom(roomCode, { type: 'client_count_update', count: currentCount }); } } catch (e) { console.error("解析消息失败", e); } }); ws.on('close', () => { if (ws.roomCode && rooms.has(ws.roomCode)) { const room = rooms.get(ws.roomCode); room.delete(ws); console.log(\`一个客户端离开了房间: \${ws.roomCode}\`); if (room.size > 0) { const currentCount = room.size; broadcastToRoom(ws.roomCode, { type: 'client_count_update', count: currentCount }); } else { rooms.delete(ws.roomCode); } } }); });
function broadcastToRoom(roomCode, data) { if (rooms.has(roomCode)) { rooms.get(roomCode).forEach(client => { if (client.readyState === WebSocket.OPEN) { client.send(JSON.stringify(data)); } }); } }
const PORT = process.env.PORT || 3000;
server.listen(PORT, () => { console.log(\`服务器正在运行于端口 \${PORT}\`); });