...
const checkAuth = (req, res, next) => {
    ...
    // 如果未认证，直接返回403 Forbidden错误
    res.status(403).send('Forbidden: Please log in using your secret link.');
};
...
app.get('/logout', (req, res) => {
    req.session.destroy(err => {
        ...
        // 不再跳转到/login，而是直接显示一条信息
        res.send('您已成功登出。请通过您的秘密链接重新登录。');
    });
});
