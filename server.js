...
const questionSchema = new mongoose.Schema({
    ...
    createdAt: { type: Date, default: Date.now } // 确保时间戳存在
});
...
// --- 路由定义 ---
app.get('/', adminAuth, (req, res) => res.sendFile(path.join(__dirname, 'public', 'dashboard.html')));
...
// --- API 接口定义 ---
app.post('/api/sessions', adminAuth, async (req, res) => { try { const newSession = new Session({ name: req.body.name }); await newSession.save(); res.status(201).json(newSession); } catch (e) { res.status(500).json({ message: '创建失败' }); } });
app.get('/api/sessions', adminAuth, async (req, res) => { try { const sessions = await Session.find().sort({ createdAt: -1 }); res.json(sessions); } catch (e) { res.status(500).json({ message: '获取列表失败' }); } });
...
app.post('/api/ask/:code', async (req, res) => {
    ...
    try {
        ...
        const newQuestion = new Question({ text: question, name: name || '匿名', sessionId: session._id, ipAddress: req.ip });
        await newQuestion.save();
        // *** 核心修改 *** 广播给前端的内容，包含 createdAt
        broadcastToRoom(session.code, {
            ...
            payload: {
                ...
                createdAt: newQuestion.createdAt // <-- 包含时间戳
            }
        });
        res.status(200).json({ message: '问题已收到' });
    } catch (e) { console.error("提交问题时出错:", e); res.status(500).json({ message: '提交失败' }); }
});

app.delete('/api/questions/:id', adminAuth, async (req, res) => { try { const questionId = req.params.id; if (!mongoose.Types.ObjectId.isValid(questionId)) { return res.status(400).json({ message: '无效的问题ID' }); } const deletedQuestion = await Question.findByIdAndDelete(questionId); if (!deletedQuestion) { return res.status(404).json({ message: '问题未找到或已被删除' }); } const session = await Session.findById(deletedQuestion.sessionId); if (session) { broadcastToRoom(session.code, { type: 'question_deleted', payload: { questionId: deletedQuestion._id } }); } res.status(200).json({ message: '问题已成功删除' }); } catch (e) { console.error("删除问题失败:", e); res.status(500).json({ message: '服务器错误' }); } });
app.get('/api/questions', adminAuth, async (req, res) => { const { start, end } = req.query; if (!start || !end) { return res.status(400).json({ message: '必须提供开始(start)和结束(end)时间参数' }); } try { const query = { createdAt: { $gte: new Date(start), $lte: new Date(end) } }; const filteredQuestions = await Question.find(query).sort({ createdAt: 1 }); res.status(200).json(filteredQuestions); } catch (error) { res.status(500).json({ message: '服务器错误' }); } });
